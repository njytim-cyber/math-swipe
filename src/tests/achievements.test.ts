...
import { checkAchievements, type AchievementStats } from '../utils/achievements';
...
describe('achievements.ts', () => {
    ...
    it('awards streak-20 when best streak hits 20', () => {
        const stats: AchievementStats = { ...baseStats, bestStreak: 20 };
        const unlocked = checkAchievements(stats, new Set());
    ...
    it('awards sharphooter badge (90%+ over 50 questions)', () => {
        const stats: AchievementStats = { ...baseStats, totalSolved: 50, totalCorrect: 50 };
        const unlocked = checkAchievements(stats, new Set());
        expect(unlocked).toContain('sharpshooter');
    });

    it('does not recursively award already unlocked badges', () => {
        const stats: AchievementStats = { ...baseStats, bestStreak: 10 };
        const prevUnlocked = new Set(['streak-10', 'streak-5']);
        const newlyUnlocked = checkAchievements(stats, prevUnlocked);
    ...
    it('awards dedicated badge for 7 days played', () => {
        const stats: AchievementStats = { ...baseStats, dayStreak: 7 };
        const unlocked = checkAchievements(stats, new Set());
