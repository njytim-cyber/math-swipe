...
export function TricksPage({ onLessonActive }: Props) {
    ...
    const startLongPress = useCallback((trick: MagicTrick) => {
    ...
    const masteredCount = MAGIC_TRICKS.filter(t => mastered.has(t.id)).length;
    const trickMap = Object.fromEntries(MAGIC_TRICKS.map(t => [t.id, t]));
